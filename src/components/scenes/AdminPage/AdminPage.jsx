...
const AdminPage = () => {
  ...
  return (
    ...
    const [setProjectGallery, {isLoading: isLoadingGallery}] = useAddProjectGalleryMutation();
    const [setProjectMain, {isLoading: isLoadingMain}] = useAddProjectMainMutation();
    ...
    const [list, setList] = useState(["New Project", "Completed Projects", "Under Construction"])
    ...
    if (isLoadingGallery || isLoadingMain || isLoadingDelete || isLoading || isFetching) {
        return <Loader/>;
    }
    ...
    return (
        <div className={s.admin}>
            <Flex gap="2rem" className={s.adminButtons}>
                <button
                    style={admin === 'add' ? {backgroundColor: 'gray', cursor: 'default'} : {}}
                    className={s.button}
                ...
                <button
                    style={admin === 'delete' ? {backgroundColor: 'gray', cursor: 'default'} : {}}
                    className={s.button}
                ...
                <button
                    style={admin === 'messages' ? {backgroundColor: 'gray', cursor: 'default'} : {}}
                    className={s.button}
                ...
                <Box>
                    <Flex flexDirection="column" gap="2rem">
                        <Flex flexDirection="column">
                            ...
                            <input className={s.input} onChange={e => setTitle(e.target.value)}/>
                        </Flex>
                        <Flex flexDirection="column">
                            <Text>Project Description</Text>
                            <textarea className={s.input} onChange={e => setDesc(e.target.value)}/>
                        </Flex>
                        <Flex flexDirection="column">
                            <Text>Location</Text>
                            <input className={s.input} onChange={e => setLocation(e.target.value)}/>
                        </Flex>
                        <Flex flexDirection="column">
                            ...
                            <select
                                ...
                                className={s.input}>
                                {
                                    list.map((address, key) => <option key={key} value={key}>{address}</option>)
                                }
                            </select>
                        </Flex>
                        <Flex flexDirection="column">
                            <Text>Main image of project (jpg, jpeg, png)</Text>
                            <Flex alignItems="center" justifyContent="center" padding="2rem" className={s.border}>
                                <ImageUploading
                                    value={main}
                                    ...
                                    {({
                                        ...
                                        <Flex flexDirection="column" alignItems="center">
                                            <Flex gap="2rem" justifyContent="center">
                                                ...
                                                <button className={s.button} onClick={onImageRemoveAll}>Remove all
                                                    images
                                                </button>
                                            </Flex>
                                            <Flex margin="1rem" gap="1rem" flexWrap="wrap">
                                                {imageList.map((image, index) => (
                                                    <Flex alignSelf="flex-end" flex="1 0 21%" flexDirection="column"
                                                          alignItems="center" key={index} className="image-item">
                    ...
                    <button className={s.finalButton} onClick={handleSubmit}>Add project</button>
                </Box>
            }
            {admin === 'delete' &&
            <Box>
                <Flex flexDirection="column" gap="2rem">
                    ...
                    <Flex flexDirection="column" gap="2rem">
                        {projects?.map(el => (
                            <Flex key={el.id} flexDirection="column" alignItems="center" gap="2rem" className={s.item}>
                                <Text maxW="100%" style={{wordWrap: "break-word"}}>{el.title}</Text>
                                <img className={s.image} src={`${API_URL}image/${el.title}/main/0.jpeg`} alt={el.title}/>
                                <button
                                    className={s.button}
            ...
            {admin === 'messages' &&
                <Box>
                    <Flex flexDirection="column" gap="2rem">
                        ...
                        <Flex flexDirection="column" gap="2rem">
                            {messages?.map(el => (
                                <Flex key={el.id} flexDirection="column" alignItems="flex-start" gap="2rem" className={s.item}>
                                    <Text maxW="100%" style={{wordWrap: "break-word"}}>First Name: {el.firstName}</Text>
                                    <Text maxW="100%" style={{wordWrap: "break-word"}}>Last Name: {el.lastName}</Text>
                                    <Text maxW="100%" style={{wordWrap: "break-word"}}>Email: {el.email}</Text>
                                    <Text maxW="100%" style={{wordWrap: "break-word"}}>Phone: {el.phone}</Text>
                                    <Text maxW="100%" style={{wordWrap: "break-word"}}>Message: {el.message}</Text>
                                </Flex>
                            ))}
                        </Flex>
                    </Flex>
